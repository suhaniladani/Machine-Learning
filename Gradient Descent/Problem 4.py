...
optimizer_train = tf.train.GradientDescentOptimizer(.01).minimize(loss)
...
for step in range(2,epoch):
    _, l=sess.run([optimizer_train,loss], feed_dict={x_p: x_input, y_p:[t for t in y_input.values]})
    if step%1000==0:
        print (l)
...
print ("Accuracy:" , sess.run(accuracy,feed_dict={x_p: X_test, y_p:[t for t in y_test.values]}))
